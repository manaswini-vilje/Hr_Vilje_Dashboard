...
import { Users, Shield, Activity, Plus, MoreVertical, X } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const getRoleBadge = (role: string) => {
    const roleConfig: Record<string, { className: string }> = {
      ...
      'HR Manager': { className: 'bg-blue-100 text-blue-700 border-blue-200' },
      'Employee': { className: 'bg-gray-100 text-gray-700 border-gray-200' },
    };

    const config = roleConfig[role] || roleConfig['Employee'];
    return <Badge className={config.className}>{role}</Badge>;
  };
